...
// ===
// Minor, irrelevant changes for DEV-CPP by ASkr; www.askrprojects.net
// Used this for SiSeLi, the ScilabSerialLib.
// ASkr, 10/2010
//
...
// is generated when using the _RPTF and // __ASSERTE macros.
...
//			_RPTF0(_CRT_WARN,"CSerial::Open - Unable to create event\n");
	...
	if (dwInQueue || dwOutQueue)
	{
		...
		// 16 bytes large.
		// __ASSERTE(dwInQueue >= 16);
		// __ASSERTE(dwOutQueue >= 16);
...
//		_RPTF0(_CRT_WARN,"CSerial::WaitEvent - Overlapped I/O is disabled, specified parameters are illegal.\n");
	...
	// __ASSERTE(!m_hevtOverlapped || HasOverlappedIoCompleted(lpOverlapped));
...
//		_RPTF0(_CRT_WARN,"CSerial::GetParity - Device is not opened\n");
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetParity - Unable to obtain DCB information\n");
		return EParUnknown;
	}
...
CSerial::EStopBits CSerial::GetStopBits (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::GetStopBits - Device is not opened\n");
		return EStopUnknown;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetStopBits - Unable to obtain DCB information\n");
		return EStopUnknown;
	}
...
DWORD CSerial::GetEventMask (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::GetEventMask - Device is not opened\n");
		return 0;
	}
...
BYTE CSerial::GetEventChar (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::GetEventChar - Device is not opened\n");
		return 0;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetEventChar - Unable to obtain DCB information\n");
		return 0;
	}
...
CSerial::EHandshake CSerial::GetHandshaking (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::GetHandshaking - Device is not opened\n");
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetHandshaking - Unable to obtain DCB information\n");
		return EHandshakeUnknown;
	}
...
LONG CSerial::Write (const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	// __ASSERTE(!lpOverlapped || pdwWritten);
	...
	*pdwWritten = 0;
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::Write - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!m_hevtOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::Write - Overlapped I/O is disabled, specified parameters are illegal.\n");
		return m_lLastError;
	}
	...
	if (!::WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped))
	{
		...
		if (lLastError != ERROR_IO_PENDING)
		{
			...
			// Issue an error and quit
			// _RPTF0_CRT_WARN,"CSerial::Write - Unable to write the data\n");
			return m_lLastError;
		}
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				...
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwWritten,FALSE))
				{
					...
					// _RPTF0_CRT_WARN,"CSerial::Write - Overlapped completed without result\n");
					return m_lLastError;
				}
				break;
			...
			default:
				...
				// Issue an error and quit
				// _RPTF0_CRT_WARN,"CSerial::Write - Unable to wait until data has been sent\n");
				return m_lLastError;
			}
		}
	}
...
LONG CSerial::Read (void* pData, size_t iLen, DWORD* pdwRead, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	// __ASSERTE(!lpOverlapped || pdwRead);
	...
	*pdwRead = 0;
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::Read - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!m_hevtOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::Read - Overlapped I/O is disabled, specified parameters are illegal.\n");
		return m_lLastError;
	}
	...
	if (!::ReadFile(m_hFile,pData,iLen,pdwRead,lpOverlapped))
	{
		...
		if (lLastError != ERROR_IO_PENDING)
		{
			...
			// Issue an error and quit
			// _RPTF0_CRT_WARN,"CSerial::Read - Unable to read the data\n");
			return m_lLastError;
		}
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				...
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwRead,FALSE))
				{
					...
					// _RPTF0_CRT_WARN,"CSerial::Read - Overlapped completed without result\n");
					return m_lLastError;
				}
				break;
			...
			default:
				...
				// Issue an error and quit
				// _RPTF0_CRT_WARN,"CSerial::Read - Unable to wait until data has been read\n");
				return m_lLastError;
			}
		}
	}
...
LONG CSerial::Purge()
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::Purge - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::PurgeComm(m_hFile, PURGE_TXCLEAR | PURGE_RXCLEAR))
	{
		// Set the internal error code
		m_lLastError = ::GetLastError();
		// _RPTF0_CRT_WARN,"CSerial::Purge - Overlapped completed without result\n");
	}
...
LONG CSerial::Break (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::Break - Device is not opened\n");
		return m_lLastError;
	}
...
CSerial::EError CSerial::GetError (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::GetError - Device is not opened\n");
	...
	if (!::ClearCommError(m_hFile,&dwErrors,0))
	{
		...
		// Issue an error and quit
		// _RPTF0_CRT_WARN,"CSerial::GetError - Unable to obtain COM status\n");
		return EErrorUnknown;
	}
...
bool CSerial::GetCTS (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetCTS - Unable to obtain the modem status\n");
		return false;
	}
...
bool CSerial::GetDSR (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetDSR - Unable to obtain the modem status\n");
		return false;
	}
...
bool CSerial::GetRing (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetRing - Unable to obtain the modem status");
		return false;
	}
...
bool CSerial::GetRLSD (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		// _RPTF0_CRT_WARN,"CSerial::GetRLSD - Unable to obtain the modem status");
		return false;
	}
